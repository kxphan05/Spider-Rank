...
recommendations plus one clarifying question each turn. 
...
API; every turn is `prompt_tokens = completion_tokens = 0`. The **Hugging Face Hub** 
is used once to download model weights; evaluation then runs fully offline, verified 
by a strict offline preflight check..
FastAPI powers an optional, unscored local demo UI only.

## Libraries and frameworks used

- **sentence-transformers** and **transformers** (Hugging Face) run the
frozen encoder and cross-encoder on **PyTorch** (CPU-only).
- **NumPy** for
fusion and scoring math.
- **SQLite FTS5** (stdlib) for BM25 and phrase
retrieval over the full catalog.
- **pytest**/**ruff** for
tests/lint.
- **Streamlit** and **FastAPI + Uvicorn + Pydantic** for two
optional, unscored demo UIs.
- Models: `BAAI/bge-small-en-v1.5` (dense
...
metadata only).
- The organizer's 200-sample public evaluation set and
unmodified local evaluator, used read-only for measurement.
- The only
